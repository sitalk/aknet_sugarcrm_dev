...
class ContactsController extends SugarController
{
	public function pre_editview()
        ...
        if (empty($this->bean->id) && !empty($_REQUEST['return_module']) &&$_REQUEST['return_module'] == 'Aknet_PrivateLeads' ) 
        {
 
            $private_lead=new Aknet_PrivateLeads();
            $private_lead->retrieve($_REQUEST['return_id']);
            foreach($private_lead->field_defs as $key=>$value)
            {
                if ($key == 'id' or $key=='deleted' )continue;
                if (isset($this->bean->field_defs[$key])) 
                {
                    $this->bean->$key = $private_lead->$key;
                    $this->view = 'edit';
                }
            }
            $_POST['is_converted']=true;
        }
        if (empty($this->bean->id) && !empty($_REQUEST['return_module']) &&$_REQUEST['return_module'] == 'Aknet_BusinessLeads' ) 
        {
 
            $private_lead=new Aknet_BusinessLeads();
            $private_lead->retrieve($_REQUEST['return_id']);
            foreach($private_lead->field_defs as $key=>$value)
            {
                if ($key == 'id' or $key=='deleted' )continue;
