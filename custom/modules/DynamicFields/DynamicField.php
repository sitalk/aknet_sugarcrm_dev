  ...
  function getJOIN( $expandedList = false , $includeRelates = false, &$where = false){
   ...
   function fill_relationships(){
    ...
    function addFieldObject(&$field){
        ...
        $fmd->type = $field->type;
        $fmd->help = $field->help;
        $fmd->module_name_input = $field->module_name_input;
        if (!empty($field->len))
     ...
     * @return boolean
     */
    function addField($name,$label='', $type='Text',$max_size='255',$required_option='optional', $default_value='', $ext1='', $ext2='', $ext3='',$audited=0, $mass_update = 0 , $ext4='', $module_name_input='', $help='', $duplicate_merge=0, $comment=''){
        require_once('modules/DynamicFields/templates/Fields/TemplateField.php');
        ...
        $field->ext4 = $ext4;
        $field->help = $help;
        $field->$module_name_input = $module_name_input;
