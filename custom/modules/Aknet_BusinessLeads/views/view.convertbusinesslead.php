...
class ViewConvertBusinessLead extends SugarView
{
    protected $fileName = "custom/modules/Aknet_BusinessLeads/metadata/convertdefs.php";
 ...
 *
 * 
pre_display function taken out
 * 
 * 
 * 
 */ 
 ...
 public function display()
    {
        ...
        $editviewFile = 'custom/modules/Aknet_BusinessLeads/metadata/editviewdefs.php';
        ...
        foreach($viewdefs['Aknet_BusinessLeads']['EditView']['panels'] as $panel_index => $section){
        ...
        $smarty->assign("module", "Aknet_BusinessLeads");
        $smarty->assign("view", "convertbusinesslead");
        $smarty->assign("bean", $this->focus);
        $smarty->assign("record_id", $this->focus->id);
        global $mod_strings;
        $smarty->assign('MOD', $mod_strings);
        $smarty->display("custom/modules/Aknet_BusinessLeads/tpls/ConvertLeadHeader.tpl");
        ...
        foreach($this->defs as $module => $vdef)
        {
            ...
            $ev->setup($module, $focus, $this->medataDataFile, "custom/modules/Aknet_BusinessLeads/tpls/ConvertLead.tpl", false);
            $ev->process();
        ...
        $smarty->assign("bean", $this->focus);
        $smarty->display("custom/modules/Aknet_BusinessLeads/tpls/ConvertLeadFooter.tpl");
    }

    protected function getRecord()
    {
        $this->focus = new Aknet_BusinessLeads();
    ...
    protected function handleSave()
    {
        ...
        foreach ($beans as $bean)
        {
            if (!empty($lead))
            {
                ...
                $ConvertBusinessLeadRel = $this->findRelationship($bean, $lead);
                if (!empty($leadsRel))
                {
